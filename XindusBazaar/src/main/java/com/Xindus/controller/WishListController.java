...
public class WishListController {
	...
	public ResponseEntity<WishLists> getWishList(@PathVariable Integer wishListId) {
		log.info("Inside getWishList Method of WishListController.");
		WishLists wishList = wishListService.getWishList(wishListId);

		return new ResponseEntity<WishLists>(wishList, HttpStatus.ACCEPTED);
	}

	// Method for Adding item to User's WishList.
	@PostMapping("/{wishListId}/{itemId}")
	public ResponseEntity<WishLists> addWishListItem(@PathVariable Integer wishListId, @PathVariable Integer itemId) {
		log.info("Inside addWishListItem Method of WishListController.");

		WishLists wishList = wishListService.addToWishList(wishListId, itemId);
		return new ResponseEntity<WishLists>(wishList, HttpStatus.CREATED);
	}

	// Method for Removing item for User's WishList.
	@DeleteMapping("/{wishListId}/{itemId}")
	public ResponseEntity<WishLists> deleteWishListItem(@PathVariable Integer wishListId,
			@PathVariable Integer itemId) {
		log.info("Inside deleteWishListItem Method of WishListController.");

		WishLists wishList = wishListService.removeFromWishList(wishListId, itemId);
